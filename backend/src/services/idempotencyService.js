...
const claimPayment = async (req, payload) => {
  ...
  if (!key || key.trim().length < 8 || key.length > 255) {
    const error = new Error(
      'Provide an Idempotency-Key header (8-255 characters) for this payment'
    );
  ...
  try {
    ...
    const duplicate = await PaymentRequest.findOne({
      user: req.user._id,
      key,
    });
    ...
    if (
      ...
      return { replay: duplicate.response };
    }
...
const completePayment = (request, response, transaction, session) =>
  PaymentRequest.findByIdAndUpdate(
    request._id,
    {
      status: 'COMPLETED',
      response,
...
const failPayment = (request, message) =>
  PaymentRequest.findByIdAndUpdate(request._id, {
    status: 'FAILED',
    response: {
      message,
    },
